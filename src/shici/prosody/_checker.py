...
FREQ = ({ch: i for i, ch in enumerate(open(_FREQ_FILE, encoding="utf-8").read().strip())}
        if os.path.exists(_FREQ_FILE) else {})
...
def cmd_template(args):
    ...
    name5 = {4:"五绝", 8:"五律"}; name7 = {4:"七绝", 8:"七律"}
...
def cmd_tone(args):
    for ch in args.chars:
        if ch in PUNC or not ch.strip(): continue
        p = readings(ch, False); x = readings(ch, True)
        ...
        for ch, lst in PING.items():
            for yun, diao in lst:
                if yun == target and diao in ("上平", "下平") and ch not in seen:
                    seen.add(ch); chars.append(ch)
    if not chars:
        print(f"未找到韵部「{target}」。平水韵平声部示例：一东 二冬 … 十五删 / 一先 … ；"
              f"新韵：一麻 二波 … 十四姑。"); return
...
def is_tewa(line, L, yan):
    ...
    if yan == 5 and len(line) >= 5:                       # 平[平仄平仄]
        return (can(line[1], "平") and can(line[2], "仄")
                and can(line[3], "平") and can(line[4], "仄"))
    if yan == 7 and len(line) >= 7:                       # 仄仄平[平仄平仄]
        return (can(line[3], "平") and can(line[4], "仄")
                and can(line[5], "平") and can(line[6], "仄"))
    return False
...
def cmd_check(args):
    ...
    for i, line in enumerate(lines):
        ...
        # 押韵句（对句 + 首句入韵时的首句）
        is_rhyme = (patt[-1] == "平") and (i % 2 == 1 or (i == 0 and patt[-1]=="平" and L in ("B","D")))
        ...
        print(f"  {i+1} {line}  [{got}]  谱{patt}({L}){rhyme_tag}{note}  {tag}{(' '+'；'.join(marks)) if marks else ''}")
        if marks: issues.append((i+1, marks))
    ...
    if rhyme_chars:
        ...
        if common:
            ...
            detail = "；".join(f"{c}={'/'.join(sorted(ping_yunbu(c,xin))) or '非平/未知'}" for _,c in rhyme_chars)
    ...
    print()
    print(f"判定首句式：{ {'A':'仄起·首句不入韵','B':'平起·首句入韵','C':'平起·首句不入韵','D':'仄起·首句入韵'}.get(L0,'?') }"
          f"（标准谱字母序 {''.join(seq)}）")
    nbad = len(issues)
...
def main():
    ...
    p = sub.add_parser("tone"); p.add_argument("chars"); p.set_defaults(func=cmd_tone)
    p = sub.add_parser("yun"); p.add_argument("yunbu"); p.add_argument("--xin", action="store_true"); p.add_argument("--all", action="store_true"); p.set_defaults(func=cmd_yun)
